...
public class EmailJobHelper {
    ...
    private EmailSettings setEmailSettings() {
        return new EmailSettings(
                "nsa-stundenplan@gmx.de",
                "nsa-stundenplan",
                "nsa-stundenplan@gmx.de",
                "NSA - Stundenplan Abweichung",
                "smtp.gmx.net");
    }

    public final void sendMailToGroups(final EntityList entityList) {

        final ArrayList<EmailObject> emailList = this.createEmailObjects(entityList);
        new EmailJob().sendMail(this.setEmailSettings(), emailList);
    }

    private ArrayList<EmailObject> createEmailObjects(final EntityList entityList) {
